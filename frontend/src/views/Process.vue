<template>
  <div class="process-page">
    ...
    <div class="main-content">
      <!-- Bên trái: hiển thị đồ thị thời gian thực -->
      <div class="left-panel" :class="{ 'full-screen': isFullScreen }">
        ...
        <div class="graph-container" ref="graphContainer">
          <!-- Trực quan hóa đồ thị (hiển thị ngay khi có dữ liệu) -->
          <div v-if="graphData" class="graph-view">
            <svg ref="graphSvg" class="graph-svg"></svg>
            <!-- Gợi ý khi đang xây dựng -->
            <div v-if="currentPhase === 1" class="graph-building-hint">
              <span class="building-dot"></span>
              Updating in real time...
            </div>
            
            <!-- Bảng chi tiết nút/cạnh -->
          ...
          <div v-else-if="currentPhase < 1" class="graph-waiting">
            ...
            <p class="waiting-hint">Graph building will start automatically when generation finishes</p>
          </div>
          
          <!-- Đang xây dựng nhưng chưa có dữ liệu -->
      ...
      <!-- Bên phải: chi tiết quy trình xây dựng -->
            ...
            <div class="phase-detail">
              <div class="detail-section">
                ...
                <div class="detail-content">
                  After document upload, the LLM analyzes content and generates an ontology structure for public-opinion simulation (entity types + relation types)
                </div>
              </div>
              
              <!-- Tiến độ tạo ontology -->
          ...
          <div class="process-phase" :class="{ 'active': currentPhase === 1, 'completed': currentPhase > 1 }">
            ...
            <div class="phase-detail">
              <div class="detail-section">
                <div class="detail-label">API Description</div>
                <div class="detail-content">
                  Based on the generated ontology, chunk the document and call Zep API to build the knowledge graph and extract entities/relations
                </div>
              </div>
              
              <!-- Chờ ontology hoàn tất -->
...
// ID dự án hiện tại (có thể đổi từ 'new' sang ID thực tế)
...
const ontologyProgress = ref(null) // Tiến độ tạo ontology
const currentPhase = ref(-1) // -1: đang tải lên, 0: tạo ontology, 1: xây dựng đồ thị, 2: hoàn tất
const selectedItem = ref(null) // Nút hoặc cạnh được chọn
...
const goToNextStep = () => {
  // TODO: Chuyển sang bước thiết lập môi trường
...
// Khởi tạo - xử lý dự án mới hoặc tải dự án đã có
const initProject = async () => {
  const paramProjectId = route.params.projectId
  
  if (paramProjectId === 'new') {
    // Dự án mới: lấy dữ liệu đang chờ tải lên từ store
...
// Xử lý dự án mới - gọi API ontology/generate
const handleNewProject = async () => {
  ...
  try {
    loading.value = true
    currentPhase.value = 0 // Giai đoạn tạo ontology
    ...
    if (response.success) {
      // Xóa dữ liệu chờ upload
      clearPendingUpload()
      
      // Cập nhật ID dự án và dữ liệu
      ...
      // Tự động bắt đầu xây dựng đồ thị
...
const startBuildGraph = async () => {
  try {
    ...
    if (response.success) {
      ...
      // Lưu task_id để polling
      const taskId = response.data.task_id
      
      // Bắt đầu polling dữ liệu đồ thị (độc lập với polling trạng thái tác vụ)
      startGraphPolling()
      
      // Bắt đầu polling trạng thái tác vụ
...
// Bộ đếm thời gian polling dữ liệu đồ thị
let graphPollTimer = null

// Bắt đầu polling dữ liệu đồ thị
const startGraphPolling = () => {
  // Lấy dữ liệu ngay lập tức một lần
  fetchGraphData()
  
  // Tự động lấy dữ liệu đồ thị mỗi 10 giây
  graphPollTimer = setInterval(async () => {
    await fetchGraphData()
  }, 10000)
}

// Làm mới đồ thị thủ công
...
const fetchGraphData = async () => {
  try {
    // Lấy thông tin dự án trước để lấy graph_id
    ...
    if (projectResponse.success && projectResponse.data.graph_id) {
      ...
      if (graphResponse.success && graphResponse.data) {
        ...
        // Cập nhật render khi dữ liệu thay đổi
...
const startPollingTask = (taskId) => {
  // Thực thi truy vấn ngay một lần
  pollTaskStatus(taskId)
  
  // Sau đó polling định kỳ
  ...
  try {
    ...
    if (response.success) {
      ...
      if (task.status === 'completed') {
        ...
        // Cập nhật tiến độ hiển thị sang trạng thái hoàn tất
        buildProgress.value = {
          progress: 100,
          message: 'Build complete, loading graph...'
        }
        
        // Tải lại dữ liệu dự án để lấy graph_id
        const projectResponse = await getProject(currentProjectId.value)
        if (projectResponse.success) {
          projectData.value = projectResponse.data
          
          // Tải đầy đủ dữ liệu đồ thị cuối cùng
...
const renderGraph = () => {
  ...
  const nodes = nodesData.map(n => ({
    ...
    rawData: n // Lưu dữ liệu gốc
  }))
  
  // Tạo tập ID nút để lọc cạnh hợp lệ
  ...
  // Vẽ cạnh (bao gồm đường rộng trong suốt để click)
  ...
  // Đường rộng trong suốt để click
  ...
  // Nhấn vùng trống để đóng bảng chi tiết
  svg.on('click', () => {
    closeDetailPanel()
  })
  
  simulation.on('tick', () => {
    // Cập nhật vị trí tất cả cạnh (gồm đường hiển thị và vùng click trong suốt)
...
// Theo dõi thay đổi dữ liệu đồ thị
watch(graphData, () => {
  if (graphData.value) {
...
/* Hiển thị quan hệ trong chi tiết cạnh */
.edge-relation {
  display: flex;
  align-items: center;
  flex-wrap: wrap;
