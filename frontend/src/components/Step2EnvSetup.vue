    ...
    <div class="scroll-container">
      ...
      <div class="step-card" :class="{ 'active': phase === 1, 'completed': phase > 1 }">
        ...
        <div class="card-content">
          ...
          <p class="description">
            Combine context and automatically call tools to extract entities and relationships from the knowledge graph, initialize simulated agents, and assign them unique behaviors and memories based on real-world seeds
          </p>
          ...
          <div v-if="profiles.length > 0" class="stats-grid">
            ...
            <div class="stat-card">
              ...
              <span class="stat-label">Expected Total Agent Count</span>
            </div>
            <div class="stat-card">
              <span class="stat-value">{{ totalTopicsCount }}</span>
              <span class="stat-label">Current Related Topics from Real-World Seeds</span>
            </div>
          </div>
      ...
      <div class="step-card" :class="{ 'active': phase === 3, 'completed': phase > 3 }">
        ...
        <div class="card-content">
          <p class="api-note">POST /api/simulation/prepare</p>
          <p class="description">
            Automatically generate initial activation events and trending topics based on the narrative direction to guide the initial state of the simulation world
          </p>
      ...
      <div class="step-card" :class="{ 'active': phase === 4 }">
        ...
        <div class="card-content">
          ...
          <!-- Simulation rounds configuration - shown only after the configuration is generated and the round count has been calculated -->
          <div v-if="simulationConfig && autoGeneratedRounds" class="rounds-config-section">
            <div class="rounds-header">
              <div class="header-left">
                <span class="section-title">Simulation Round Settings</span>
                <span class="section-desc">MiroFish automatically plans and simulates <span class="desc-highlight">{{ simulationConfig?.time_config?.total_simulation_hours || '-' }}</span> hours，each round represents <span class="desc-highlight">{{ simulationConfig?.time_config?.minutes_per_round || '-' }}</span> minutes of real-world time progression</span>
              </div>
              ...
              <div v-if="useCustomRounds" class="rounds-content custom" key="custom">
                <div class="slider-display">
                  ...
                  <div class="slider-meta-info">
                    <span>With 100 agents: estimated runtime about {{ Math.round(customMaxRounds * 0.6) }} minutes</span>
                  </div>
                </div>
              ...
              <div v-else class="rounds-content auto" key="auto">
                <div class="auto-info-card">
                  ...
                  <div class="auto-content">
                    ...
                    <div class="auto-desc">
                      <p class="highlight-tip" @click="useCustomRounds = true">For the first run, it is strongly recommended to switch to 'Custom Mode' to reduce the number of simulation rounds for faster preview and lower error risk ➝</p>
                    </div>
                  </div>
                </div>
              </div>
      ...
      <div v-if="selectedProfile" class="profile-modal-overlay" @click.self="selectedProfile = null">
        ...
        <div class="modal-body">
          ...
          <div class="modal-section">
            ...
            <p class="section-bio">{{ selectedProfile.bio || 'No description available' }}</p>
          </div>
...
// State
const phase = ref(0) // 0: Initializing, 1: Generate Persona, 2: Generate Configuration, 3: Complete
...
const customMaxRounds = ref(40)   // Recommended default: 40 rounds
...
watch(currentStage, (newStage) => {
  if (newStage === 'GenerateAgentpersona' || newStage === 'generating_profiles') {
    ...
    phase.value = 2
    // Enter configuration generation stage and start polling configuration
    if (!configTimer) {
      addLog('StartGenerate Dual-Platform Simulation Config...')
      startConfigPolling()
    }
  } else if (newStage === 'Prepare Simulation Scripts' || newStage === 'copying_scripts') {
    phase.value = 2 // Still belongs to the configuration stage
  }
})

// Calculate the auto-generated round count from the configuration (without using hard-coded defaults)
const autoGeneratedRounds = computed(() => {
  if (!simulationConfig.value?.time_config) {
    return null // Return null when the configuration has not been generated
  }
  ...
  // Ensure the maximum round count is not less than 40 (recommended) to avoid abnormal slider ranges
...
// Calculate the total number of related topics across all personas
...
// Handle click on the start simulation button
const handleStartSimulation = () => {
  // Build parameters passed to the parent component
  const params = {}
  
  if (useCustomRounds.value) {
    // The user defines a custom round count and passes the max_rounds parameter
    params.maxRounds = customMaxRounds.value
    addLog(`Start simulation, custom round count: ${customMaxRounds.value} rounds`)
  } else {
    // The user chooses to keep the auto-generated round count and does not pass the max_rounds parameter
    addLog(`Start simulation using auto-configured round count: ${autoGeneratedRounds.value} rounds`)
  }
...
const startPrepareSimulation = async () => {
  ...
  // Mark the first step as complete and start the second step
  phase.value = 1
  ...
  try {
    ...
    if (res.success && res.data) {
      if (res.data.already_prepared) {
        addLog('Detected existing completed preparation work. Using it directly')
      ...
      // Immediately set the expected total number of agents (from the prepare API return value)
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      if (data.progress_detail) {
        ...
        // Extract stage from the message
...
const fetchProfilesRealtime = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // Output Profile generation progress logs (only when the count changes)
      ...
      if (currentCount > 0 && currentCount !== lastLoggedProfileCount) {
        ...
        // If all have been generatedComplete
...
const fetchConfigRealtime = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // Xuất log giai đoạn tạo cấu hình (tránh trùng lặp)
      ...
      if (data.config_generated && data.config) {
        ...
        addLog('✓ Simulation configuration generated successfully')
        ...
        if (data.summary) {
          ...
          addLog(`  ├─ Initial posts: ${data.summary.initial_posts_count} entries`)
        ...
        stopConfigPolling()
        phase.value = 4
        addLog('✓ Environment setup completed. Ready to start simulation')
...
const loadPreparedData = async () => {
  phase.value = 2
  addLog('Loading existing prepared configuration data...')

  // Fetch Profiles one last time
  await fetchProfilesRealtime()
  addLog(`Loaded ${profiles.value.length} itemsAgentpersona`)

  // Fetch configuration (using the real-time API)
  try {
    const res = await getSimulationConfigRealtime(props.simulationId)
    if (res.success && res.data) {
      if (res.data.config_generated && res.data.config) {
        ...
        if (res.data.summary) {
          ...
          addLog(`  └─ Initial posts: ${res.data.summary.initial_posts_count} entries`)
        }
        ...
        // Cấu hình chưa được tạo, hãy bắt đầu thăm dò và chờ
...
/* Simulation Round ConfigurationStyles */
