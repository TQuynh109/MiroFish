...
const showEdgeLabels = ref(true) // Mặc định hiển thị nhãn cạnh
const expandedSelfLoops = ref(new Set()) // Các self-loop đang mở
const showSimulationFinishedHint = ref(false) // Thông báo sau khi mô phỏng kết thúc
const wasSimulating = ref(false) // Theo dõi trước đó có đang mô phỏng hay không

// Đóng thông báo kết thúc mô phỏng
const dismissFinishedHint = () => {
  showSimulationFinishedHint.value = false
}

// Theo dõi thay đổi isSimulating để phát hiện kết thúc mô phỏng
watch(() => props.isSimulating, (newValue, oldValue) => {
  if (wasSimulating.value && !newValue) {
    // Từ trạng thái mô phỏng chuyển sang không mô phỏng, hiển thị thông báo kết thúc
    showSimulationFinishedHint.value = true
  }
  wasSimulating.value = newValue
}, { immediate: true })

// Toggle trạng thái mở/đóng của self-loop
...
// Tính toán loại entity để hiển thị legend
...
const closeDetailPanel = () => {
  ...
  expandedSelfLoops.value = new Set() // Reset trạng thái mở
}
...
const renderGraph = () => {
  ...
  // Xử lý dữ liệu cạnh, tính số lượng và chỉ số cạnh giữa cùng một cặp node
  const edgePairCount = {}
  const selfLoopEdges = {} // Self-loop được nhóm theo node
  const tempEdges = edgesData
    .filter(e => nodeIds.has(e.source_node_uuid) && nodeIds.has(e.target_node_uuid))
  
  // Thống kê số cạnh giữa mỗi cặp node, thu thập self-loop
  tempEdges.forEach(e => {
    if (e.source_node_uuid === e.target_node_uuid) {
      // Self-loop - gom vào mảng
  ...
  // Ghi nhận đang xử lý cạnh thứ mấy của mỗi cặp node
  const edgePairIndex = {}
  const processedSelfLoopNodes = new Set() // Các node self-loop đã xử lý
  ...
  tempEdges.forEach(e => {
    ...
    if (isSelfLoop) {
      // Self-loop - mỗi node chỉ thêm một self-loop tổng hợp
      ...
      edges.push({
        ...
        rawData: {
          ...
          selfLoopEdges: allSelfLoops // Lưu toàn bộ thông tin chi tiết của self-loop
        }
      })
      return
    }
    ...
    // Kiểm tra hướng cạnh có trùng với hướng chuẩn hay không (source UUID < target UUID)
    const isReversed = e.source_node_uuid > e.target_node_uuid
    
    // Tính độ cong: nhiều cạnh thì tách ra, một cạnh thì là đường thẳng
    let curvature = 0
    if (totalCount > 1) {
      // Phân bố độ cong đồng đều, đảm bảo dễ phân biệt
      // Phạm vi độ cong tăng theo số lượng cạnh, càng nhiều cạnh thì phạm vi càng lớn
      const curvatureRange = Math.min(1.2, 0.6 + totalCount * 0.15)
      curvature = ((currentIndex / (totalCount - 1)) - 0.5) * curvatureRange * 2
      
      // Nếu hướng cạnh ngược với hướng chuẩn, đảo độ cong
      // Nhờ vậy tất cả cạnh được phân bố trong cùng hệ quy chiếu, tránh chồng lấn do khác hướng
      if (isReversed) {
        curvature = -curvature
      }
    }
  ...
  // Simulation - điều chỉnh khoảng cách node theo số lượng cạnh
  const simulation = d3.forceSimulation(nodes)
    .force('link', d3.forceLink(edges).id(d => d.id).distance(d => {
      // Điều chỉnh khoảng cách dựa trên số cạnh giữa cặp node này
      // Khoảng cách cơ bản là 150, mỗi cạnh thêm tăng 50
    ...
    // Thêm lực hút về trung tâm để các cụm node độc lập gom lại gần trung tâm
  ...
  // Links - dùng path để hỗ trợ đường cong
  const linkGroup = g.append('g').attr('class', 'links')
  
  // Tính đường dẫn cho đường cong
  const getLinkPath = (d) => {
    ...
    // Kiểm tra self-loop
    if (d.isSelfLoop) {
      // Self-loop: vẽ một cung tròn đi ra từ node rồi quay lại
      const loopRadius = 30
      // Bắt đầu từ bên phải node, đi một vòng rồi quay lại
      const x1 = sx + 8
      const y1 = sy - 4
      const x2 = sx + 8
      const y2 = sy + 4
      // Dùng cung tròn để vẽ self-loop (sweep-flag=1 theo chiều kim đồng hồ)
    ...
    // Tính điểm điều khiển của đường cong - điều chỉnh động theo số cạnh và khoảng cách
    const dx = tx - sx, dy = ty - sy
    const dist = Math.sqrt(dx * dx + dy * dy)
    // Độ lệch vuông góc với hướng nối, tính theo tỉ lệ khoảng cách để đảm bảo nhìn rõ
    // Càng nhiều cạnh, tỉ lệ độ lệch theo khoảng cách càng lớn
    const pairTotal = d.pairTotal || 1
    const offsetRatio = 0.25 + pairTotal * 0.05 // Giá trị cơ bản là 25%, và nó tăng thêm 5% cho mỗi cạnh bổ sung.
  ...
  // Tính trung điểm của đường cong (để đặt label)
  const getLinkMidpoint = (d) => {
    ...
    // Kiểm tra self-loop
    if (d.isSelfLoop) {
      // Vị trí label self-loop: bên phải node
      return { x: sx + 70, y: sy }
    }
    ...
    // Trung điểm của đường Bezier bậc hai tại t=0.5
    const dx = tx - sx, dy = ty - sy
    const dist = Math.sqrt(dx * dx + dy * dy)
    const pairTotal = d.pairTotal || 1
    ...
    // Công thức Bezier bậc hai B(t) = (1-t)²P0 + 2(1-t)tP1 + t²P2, với t=0.5
  ...
  const link = linkGroup.selectAll('path')
    ...
    .on('click', (event, d) => {
      ...
      // Reset style cạnh đã chọn trước đó
      linkGroup.selectAll('path').attr('stroke', '#C0C0C0').attr('stroke-width', 1.5)
      linkLabelBg.attr('fill', 'rgba(255,255,255,0.95)')
      linkLabels.attr('fill', '#666')
      // Highlight cạnh đang chọn
  ...
  // Nền label của cạnh (nền trắng giúp chữ rõ hơn)
  ...
  // Lưu reference để điều khiển hiển thị từ bên ngoài
  ...
  const node = nodeGroup.selectAll('circle')
    ...
    .call(d3.drag()
      .on('start', (event, d) => {
        // Chỉ ghi nhận vị trí, không restart simulation (phân biệt click và drag)
        d.fx = d.x
        d.fy = d.y
      ...
      .on('drag', (event, d) => {
        // Kiểm tra có thực sự bắt đầu drag hay chưa (dịch chuyển vượt ngưỡng)
        const dx = event.x - d._dragStartX
        const dy = event.y - d._dragStartY
        const distance = Math.sqrt(dx * dx + dy * dy)
        
        if (!d._isDragging && distance > 3) {
          // Chỉ khi thật sự drag mới restart simulation
          d._isDragging = true
      ...
      .on('end', (event, d) => {
        // Chỉ khi thực sự drag mới cho simulation giảm dần
        if (d._isDragging) {
    ...
    .on('click', (event, d) => {
      event.stopPropagation()
      // Reset style của tất cả node
      node.attr('stroke', '#fff').attr('stroke-width', 2.5)
      linkGroup.selectAll('path').attr('stroke', '#C0C0C0').attr('stroke-width', 1.5)
      // Highlight node được chọn
      d3.select(event.target).attr('stroke', '#E91E63').attr('stroke-width', 4)
      // Highlight các cạnh nối với node này
  ...
  simulation.on('tick', () => {
    ...
    // Cập nhật vị trí nhãn cạnh (không xoay, hiển thị ngang rõ hơn)
    linkLabels.each(function(d) {
      const mid = getLinkMidpoint(d)
      d3.select(this)
        .attr('x', mid.x)
        .attr('y', mid.y)
        .attr('transform', '') // Loại bỏ chuyển động xoay, giữ nguyên vị trí nằm ngang.
    })
    
    // Cập nhật nền nhãn cạnh
    linkLabelBg.each(function(d, i) {
      ...
      d3.select(this)
        ...
        .attr('transform', '') // Loại bỏ phép xoay
    })
  ...
  // Click vào vùng trống để đóng panel chi tiết
  svg.on('click', () => {
    selectedItem.value = null
    node.attr('stroke', '#fff').attr('stroke-width', 2.5)
    linkGroup.selectAll('path').attr('stroke', '#C0C0C0').attr('stroke-width', 1.5)
...
// Theo dõi công tắc hiển thị nhãn cạnh
