...
const selectedProject = ref(null)  // Dự án hiện được chọn (dùng cho modal)
let observer = null
let isAnimating = false  // Khóa animation, tránh nhấp nháy
let expandDebounceTimer = null  // Bộ hẹn giờ debounce
let pendingState = null  // Ghi lại trạng thái đích chờ thực thi

// Cấu hình layout thẻ - điều chỉnh sang tỷ lệ rộng hơn
const CARDS_PER_ROW = 4
const CARD_WIDTH = 280  
const CARD_HEIGHT = 280 
const CARD_GAP = 24

// Tính động style chiều cao container
const containerStyle = computed(() => {
  ...
  // Trạng thái mở rộng: tính chiều cao động theo số lượng thẻ
  ...
  // Tính chiều cao thực tế cần dùng: số hàng * chiều cao thẻ + (số hàng-1) * khoảng cách + một ít khoảng đệm dưới
...
const getCardStyle = (index) => {
  ...
  if (isExpanded.value) {
    // Trạng thái mở rộng: layout dạng lưới
    const transition = 'transform 700ms cubic-bezier(0.23, 1, 0.32, 1), opacity 700ms cubic-bezier(0.23, 1, 0.32, 1), box-shadow 0.3s ease, border-color 0.3s ease'

    const col = index % CARDS_PER_ROW
    const row = Math.floor(index / CARDS_PER_ROW)
    
    // Tính số lượng thẻ ở hàng hiện tại để căn giữa từng hàng
    ...
    // Mở rộng xuống dưới, tăng khoảng cách với tiêu đề
    ...
    const x = offset * 35
    // Điều chỉnh vị trí bắt đầu, gần tiêu đề nhưng vẫn giữ khoảng cách phù hợp
...
// Lấy class style theo tiến độ số vòng
...
// Format ngày tháng (chỉ hiển thị phần ngày)
...
// Format simulation_id để hiển thị (lấy 6 ký tự đầu)
...
// Format hiển thị số vòng (vòng hiện tại/tổng số vòng)
...
// Cắt ngắn tên file (giữ lại phần mở rộng)
...
// Điều hướng đến trang xây dựng graph (Project)
const goToProject = () => {
  ...
  observer = new IntersectionObserver(
    (entries) => {
      entries.forEach((entry) => {
        ...
        // Cập nhật trạng thái đích chờ thực thi (dù đang animation vẫn phải ghi lại trạng thái mới nhất)
        pendingState = shouldExpand
        
        // Xóa bộ hẹn giờ debounce trước đó (ý định cuộn mới sẽ ghi đè ý định cũ)
        if (expandDebounceTimer) {
          clearTimeout(expandDebounceTimer)
          expandDebounceTimer = null
        }
        
        // Nếu đang animation thì chỉ ghi trạng thái, chờ xử lý sau khi animation kết thúc
        if (isAnimating) return
        
        // Nếu trạng thái đích trùng với trạng thái hiện tại thì không cần xử lý
        if (shouldExpand === isExpanded.value) {
          pendingState = null
          return
        }
        
        // Dùng debounce để trì hoãn chuyển trạng thái, tránh nhấp nháy nhanh
        // Khi mở rộng thì delay ngắn hơn (50ms), khi thu gọn delay dài hơn (200ms) để ổn định hơn
        const delay = shouldExpand ? 50 : 200
        
        expandDebounceTimer = setTimeout(() => {
          // Kiểm tra có đang animation không
          if (isAnimating) return
          
          // Kiểm tra trạng thái chờ có còn cần thực thi không (có thể đã bị lần cuộn sau ghi đè)
          ...
          // Sau khi animation hoàn tất thì mở khóa, đồng thời kiểm tra xem có trạng thái mới đang chờ hay không
          setTimeout(() => {
            isAnimating = false
            
            // Sau khi animation kết thúc, kiểm tra xem có trạng thái chờ mới không
            if (pendingState !== null && pendingState !== isExpanded.value) {
              // Delay thêm một chút trước khi thực thi để tránh chuyển quá nhanh
              expandDebounceTimer = setTimeout(() => {
                if (pendingState !== null && pendingState !== isExpanded.value) {
                  isAnimating = true
                  isExpanded.value = pendingState
    ...
    {
      // Dùng nhiều ngưỡng để việc phát hiện mượt hơn
      threshold: [0.4, 0.6, 0.8],
      // Điều chỉnh rootMargin, co đáy viewport lên trên để phải cuộn nhiều hơn mới kích hoạt mở rộng
...
// Theo dõi thay đổi route, khi quay lại trang chủ thì tải lại dữ liệu
...
onMounted(async () => {
  // Đảm bảo DOM đã render xong rồi mới tải dữ liệu
  await nextTick()
  await loadHistory()
  
  // Chờ DOM render xong rồi mới khởi tạo observer
  setTimeout(() => {
    initObserver()
  }, 100)
})

// Nếu dùng keep-alive, tải lại dữ liệu khi component được kích hoạt
onActivated(() => {
  loadHistory()
})

onUnmounted(() => {
  // Dọn dẹp Intersection Observer
  if (observer) {
    observer.disconnect()
    observer = null
  }
  // Dọn dẹp bộ hẹn giờ debounce
  if (expandDebounceTimer) {
    clearTimeout(expandDebounceTimer)
    expandDebounceTimer = null
  }
