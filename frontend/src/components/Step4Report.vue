    ...
    <div class="main-split-layout">
      ...
      <div class="right-panel" ref="rightPanel">
        ...
        <div class="workflow-overview" v-if="agentLogs.length > 0 || reportOutline">
          ...
          <!-- Next Step Button - Hiển thị sau khi hoàn thành -->
            ...
            >
              ...
              <div class="timeline-content">
                ...
                <div class="timeline-body" :class="{ 'collapsed': isLogCollapsed(log) }" @click="toggleLogExpand(log)">
                  ...
                  <!-- Section Content Generated (Nội dung đã được tạo, nhưng toàn bộ chương có thể chưa hoàn chỉnh) -->
                  ...
                  <!-- Section Complete (Tạo chương đã hoàn thành) -->
                  ...
                  <template v-if="log.action === 'llm_response'">
                    ...
                    <!-- Khi đến đáp án cuối cùng, một thông báo đặc biệt sẽ được hiển thị. -->
...
const toggleRawResult = (timestamp, event) => {
  // Vị trí của nút lưu so với khung nhìn
  ...
  // Sau khi DOM được cập nhật, hãy điều chỉnh vị trí cuộn để giữ cho nút ở cùng một vị trí.
...
const toggleSectionCollapse = (idx) => {
  // Only completed chapters can be folded.
  if (!generatedSections.value[idx + 1]) return
...
const toolConfig = {
  'insight_forge': {
    name: 'Deep Insight',
    color: 'purple',
    icon: 'lightbulb' // Biểu tượng bóng đèn - đại diện cho insight
  },
  'panorama_search': {
    name: 'Panorama Search',
    color: 'blue',
    icon: 'globe' // Biểu tượng quả địa cầu - đại diện cho tìm kiếm toàn cảnh
  },
  'interview_agents': {
    name: 'Agent Interview',
    color: 'green',
    icon: 'users' // Biểu tượng người dùng - đại diện cho hội thoại
  },
  'quick_search': {
    name: 'Quick Search',
    color: 'orange',
    icon: 'zap' // Biểu tượng tia chớp - đại diện cho nhanh
  },
...
const parseInsightForge = (text) => {
  ...
  try {
    ...
    // Trích xuất kịch bản dự đoán
    const reqMatch = text.match(/Prediction Scenario:\s*(.+?)(?:\n|$)/)
    if (reqMatch) result.simulationRequirement = reqMatch[1].trim()
    
    // Trích xuất dữ liệu thống kê - match format "Related Facts: X"
    const factMatch = text.match(/Related Facts:\s*(\d+)/)
    ...
    // Trích xuất sub-questions - lấy đầy đủ, không giới hạn số lượng
    ...
    // Trích xuất facts quan trọng - lấy đầy đủ, không giới hạn số lượng
    ...
    // Trích xuất entities chính - bao gồm summary và số lượng facts liên quan
    const entitySection = text.match(/### \[Core Entities\]\n([\s\S]*?)(?=\n###|$)/)
    if (entitySection) {
      const entityText = entitySection[1]
      // Tách block entity theo "- **"
    ...
    // Trích xuất relationship chains - lấy đầy đủ, không giới hạn số lượng
...
const parsePanorama = (text) => {
  ...
  try {
    ...
    // Trích xuất active facts - đầy đủ, không giới hạn số lượng
    const activeSection = text.match(/### \[Active Facts\][\s\S]*?\n([\s\S]*?)(?=\n###|$)/)
    if (activeSection) {
      const lines = activeSection[1].split('\n').filter(l => l.match(/^\d+\./))
      result.activeFacts = lines.map(l => {
        // Loại bỏ số thứ tự và dấu ngoặc kép
        const factText = l.replace(/^\d+\.\s*/, '').replace(/^"|"$/g, '').trim()
        return factText
      }).filter(Boolean)
    }
    
    // Trích xuất historical facts - đầy đủ, không giới hạn số lượng
    ...
    // Trích xuất entities liên quan - đầy đủ, không giới hạn số lượng
...
const parseInterview = (text) => {
  ...
  try {
    // Trích xuất topic phỏng vấn
    const topicMatch = text.match(/\*\*Interview Topic:\*\*\s*(.+?)(?:\n|$)/)
    if (topicMatch) result.topic = topicMatch[1].trim()
    
    // Trích xuất số lượng agent (vd: "5 / 9 Agents")
    const countMatch = text.match(/\*\*Agent Count:\*\*\s*(\d+)\s*\/\s*(\d+)/)
    if (countMatch) {
    ...
    // Trích xuất lý do chọn đối tượng
    const reasonMatch = text.match(/### Selection Reason\n([\s\S]*?)(?=\n---\n|\n### Interview Records)/)
    if (reasonMatch) {
      result.selectionReason = reasonMatch[1].trim()
    }
    
    // Parse lý do từng người
    const parseIndividualReasons = (reasonText) => {
      ...
      for (const line of lines) {
        ...
        if (name) {
          // Reasons for saving the previous person
          ...
          // Continuation of the reason (excluding the concluding summary paragraph)
          currentReason.push(line.trim())
        }
      }
      
      // The reason for saving the last person
      if (currentName && currentReason.length > 0) {
        reasons[currentName] = currentReason.join(' ').trim()
      }
    ...
    // Trích xuất từng interview block
    ...
    interviewBlocks.forEach((block, index) => {
      ...
      // Trích xuất questions
      const qMatch = block.match(/\*\*Q:\*\*\s*([\s\S]*?)(?=\n\n\*\*A:\*\*|\*\*A:\*\*)/)
      if (qMatch) {
        const qText = qMatch[1].trim()
        // Problem of separating by number
        const questions = qText.split(/\n\d+\.\s+/).filter(q => q.trim())
        if (questions.length > 0) {
          // If the first question is preceded by "1.", it requires special handling.
      ...
      // Trích xuất answer (Twitter + Reddit)
      ...
      if (answerMatch) {
        ...
        if (!twitterMatch && redditMatch) {
          ...
          // No platform-specific tags (very old format), the whole answer is presented as an answer.
    ...
    // Trích xuất summary
    const summaryMatch = text.match(/### Summary & Key Insights\n([\s\S]*?)$/)
    if (summaryMatch) {
...
const parseQuickSearch = (text) => {
  ...
  try {
    ...
    // Trích xuất các sự thật liên quan - lấy đầy đủ, không giới hạn số lượng
    ...
    // Thử trích xuất thông tin node (nếu có)
    ...
    return () => h('div', { class: 'insight-display' }, [
      ...
      h('div', { class: 'insight-header' }, [
        ...
        props.result.simulationRequirement && h('div', { class: 'header-scenario' }, [
          h('span', { class: 'scenario-label' }, 'Prediction scenarios: '),
...
const InterviewDisplay = {
  ...
  setup(props) {
    ...
    // Duy trì trạng thái chọn nền tảng riêng cho từng cặp câu hỏi - câu trả lời
    const platformTabs = reactive({}) // { 'agentIdx-qIdx': 'twitter' | 'reddit' }
    
    // Lấy nền tảng hiện tại được chọn của một câu hỏi
    const getPlatformTab = (agentIdx, qIdx) => {
      const key = `${agentIdx}-${qIdx}`
      return platformTabs[key] || 'twitter'
    }
    
    // Thiết lập nền tảng cho một câu hỏi
    ...
    // Kiểm tra xem có phải là text placeholder của nền tảng hay không
    ...
    // Thử tách câu trả lời theo số thứ tự câu hỏi
    const splitAnswerByQuestions = (answerText, questionCount) => {
      ...
      // Hỗ trợ 2 định dạng đánh số:
      // 1. "Question X:" (định dạng tiếng Trung mới từ backend)
      // 2. "1. " (định dạng cũ)
      let matches = []
      let match

      // Ưu tiên format "Question X:"
      const cnPattern = /(?:^|[\r\n]+)Question(\d+)[:：]\s*/g
      ...
      // Nếu không có thì fallback sang "1."
      if (matches.length === 0) {
        const numPattern = /(?:^|[\r\n]+)(\d+)\.\s+/g
      ...
      // Nếu không tìm được hoặc chỉ có 1 phần → trả về toàn bộ
      if (matches.length <= 1) {
      ...
      // Tách nội dung theo từng số thứ tự
      const parts = []
    ...
    const getAnswerForQuestion = (interview, qIdx, platform) => {
      ...
      // Nếu tách thành công và index hợp lệ
      if (answers.length > 1 && qIdx < answers.length) {
        return answers[qIdx] || ''
      }

      // Nếu tách thất bại → câu đầu lấy full, còn lại rỗng
      return qIdx === 0 ? answer : ''
    }

    // Kiểm tra câu hỏi có câu trả lời từ 2 platform hay không (loại bỏ placeholder)
    const hasMultiplePlatforms = (interview, qIdx) => {
      ...
      // Cả hai platform đều có câu trả lời thật và khác nhau
              ...
              answerText && h('div', { class: ['qa-answer', { 'answer-placeholder': isPlaceholder }] }, [
                ...
                h('div', { class: 'qa-content' }, [
                  h('div', { class: 'qa-answer-header' }, [
                    ...
                    // Nút chuyển đổi giữa hai nền tảng (chỉ hiển thị khi có câu trả lời thực từ cả hai nền tảng）
                  ...
                  // Expand/Collapse Button（Văn bản giữ chỗ không được hiển thị)
...
// Bước đang hoạt động hiện tại (dùng để hiển thị ở phần trên)
const activeStep = computed(() => {
  const steps = workflowSteps.value
  // Tìm bước có trạng thái active hiện tại
  ...
  // Ngược lại, trả về bước đầu tiên
...
const renderMarkdown = (content) => {
  ...
  // Bọc danh sách không thứ tự
  html = html.replace(/(<li class="md-li"[^>]*>.*?<\/li>\s*)+/g, '<ul class="md-ul">$&</ul>')
  // Bọc danh sách có thứ tự
  html = html.replace(/(<li class="md-oli"[^>]*>.*?<\/li>\s*)+/g, '<ol class="md-ol">$&</ol>')

  // Dọn dẹp khoảng trắng giữa các item
  html = html.replace(/<\/li>\s+<li/g, '</li><li')
  // Dọn khoảng trắng sau thẻ mở danh sách
  html = html.replace(/<ul class="md-ul">\s+/g, '<ul class="md-ul">')
  html = html.replace(/<ol class="md-ol">\s+/g, '<ol class="md-ol">')
  // Dọn khoảng trắng trước thẻ đóng danh sách
  html = html.replace(/\s+<\/ul>/g, '</ul>')
  html = html.replace(/\s+<\/ol>/g, '</ol>')
  
  // Xử lý chữ in đậm và in nghiêng
  ...
  // Xử lý xuống dòng - dòng trống thành phân đoạn, xuống dòng đơn thành <br>
  html = html.replace(/\n\n/g, '</p><p class="md-p">')
  html = html.replace(/\n/g, '<br>')
  
  // Bọc trong thẻ đoạn văn
  ...
  // Xóa <br> trước/sau các block element
  html = html.replace(/<br>\s*(<ul|<ol|<blockquote)/g, '$1')
  html = html.replace(/(<\/ul>|<\/ol>|<\/blockquote>)\s*<br>/g, '$1')
  // Xóa trường hợp <p><br> đứng trước block element (do dòng trống dư)
  html = html.replace(/<p class="md-p">(<br>\s*)+(<ul|<ol|<blockquote|<pre|<hr)/g, '$2')
  // Xóa các <br> liên tiếp
  html = html.replace(/(<br>\s*){2,}/g, '<br>')
  // Xóa <br> trước khi bắt đầu đoạn mới sau block element
  html = html.replace(/(<\/ol>|<\/ul>|<\/blockquote>)<br>(<p|<div)/g, '$1$2')

  // Sửa lỗi đánh số danh sách có thứ tự không liên tục: khi các <ol> bị tách bởi đoạn văn, vẫn giữ số thứ tự tăng dần
...
const getLogLevelClass = (log) => {
  if (log.includes('ERROR') || log.includes('Lỗi')) return 'error'
  if (log.includes('WARNING') || log.includes('Cảnh báo')) return 'warning'
  // INFO sử dụng màu mặc định, không đánh dấu là success
  return ''
}
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      if (newLogs.length > 0) {
        newLogs.forEach(log => {
          ...
          if (log.action === 'section_complete') {
            if (log.details?.content) {
              ...
              // Tự động mở rộng chương/mục vừa được tạo
          ...
          if (log.action === 'report_complete') {
            ...
            currentSectionIndex.value = null  // Đảm bảo xóa trạng thái loading
            emit('update-status', 'completed')
            stopPolling()
            // Logic cuộn sẽ được xử lý thống nhất trong nextTick sau khi vòng lặp kết thúc
          }
        ...
        nextTick(() => {
          if (rightPanel.value) {
            // Nếu tác vụ đã hoàn thành thì cuộn lên đầu; nếu chưa thì cuộn xuống cuối để theo dõi log mới nhất
            if (isComplete.value) {
...
// Trích xuất nội dung câu trả lời cuối cùng - lấy nội dung phần từ phản hồi của LLM
const extractFinalContent = (response) => {
  if (!response) return null
  
  // Cố gắng trích xuất nội dung bên trong thẻ <final_answer>
  ...
  // Cố gắng tìm nội dung phía sau "Final Answer:" (hỗ trợ nhiều định dạng)
  ...
  // Nếu nội dung dài và có chứa định dạng markdown, hãy thử loại bỏ phần suy nghĩ rồi trả về.
  if (response.length > 300 && (response.includes('**') || response.includes('>'))) {
    // Loại bỏ phần suy nghĩ bắt đầu bằng 'Thought:'
